...
if($product_id>0){
        ...
        $product_name='余额充值';
        $product_des=isset($_POST['jinbi'])?'充值'.$_POST['jinbi'].'元':'余额充值';
}else{
...
if(empty($product_id)||empty($order_number))wp_die('获取商品信息出错,请重试或联系卖家!');
global $wpdb;
$table = $wpdb->order;
...
if ($user_id&&!$order) {
    ...
    $insert_order_id=$wpdb->insert($wpdb->order,array('product_id' =>$product_id,//产品id
                                'order_number'=>$order_number,
                                'order_status' =>1,//-1,订单已取消，0购物车,1订单待付款,2订单待发货，3订单已发货，4已收货，5交易完成
                                'user_id'=>$user_id,
                                'price'=>$_POST['jinbi'],
                                'order_time'=>$today_time,
                                'product_count'=>1
                            ),
                            array('%d','%s','%d','%d','%d','%s','%d'));
        if($insert_order_id){
            $order_message= '';//$wpdb->insert_id会输出插入id
        }else{
            $order_message= '订单有错误！';
        }
        $order=$wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->order where product_id=%d and order_number=%s and user_id=%d;",$product_id,$order_number,$user_id),OBJECT,0);
...
/**************************请求参数**************************/
        ...
        //收货人姓名
        $receive_name = $user_dizhi_obj->lianxi_name;
        //如：张三

        //收货人地址
        $receive_address = $user_dizhi_obj->city.'，'.$user_dizhi_obj->dizhi_detail;
        //如：XX省XXX市XXX区XXX路XXX小区XXX栋XXX单元XXX号

        //收货人邮编
        $receive_zip = $user_dizhi_obj->youbian;
        //如：123456

        //收货人电话号码
        $receive_phone = $user_dizhi_obj->telphone;
        //如：0571-88158090

        //收货人手机号码
        $receive_mobile = $user_dizhi_obj->telphone;
        //如：13312341234
